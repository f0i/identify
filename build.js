...
esbuild
  .build({
    entryPoints: ["src/frontend/candid.ts"], // Replace with your entry file
    bundle: true, // Bundle all dependencies into a single file
    outfile: "out/frontend/candid.js", // Output file
    minify: true, // Minify the output for production
    sourcemap: true, // Generate source maps for debugging
    target: "esnext", // The JavaScript version to target
    platform: "browser", // Ensure it's bundled for the browser
  ...
  .then(() => {
    const input = "src/frontend/candid.html";
    const outfile = "out/frontend/candid.html";

    // Add a timestamp as version ID
    let html = fs.readFileSync(input, "utf8");
    html = html.replace(
      /<script src=".\/candid\.js"/,
      `<script src="./candid\.js?v=${Date.now()}"`,
    );
